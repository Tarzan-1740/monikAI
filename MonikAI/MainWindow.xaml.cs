...
namespace MonikAI
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private readonly object sayingLock = new object();
        private bool preferAIResponses = true;
        ...
        public MainWindow()
        {
            ...
            if (string.IsNullOrWhiteSpace(MonikaiSettings.Default.Language) || MonikaiSettings.Default.Language == "English")
            {
            ...
            Task.Run(async () => 
            {
                ...
                if (!await this.lmStudioService.IsLMStudioAvailable())
                {
                    ...
                    await Task.Delay(3000); // Esperar a que inicie
                }
            });
        ...
        {
            if (!this.preferAIResponses || this.lmStudioService == null)
            {
                lock (this.sayingLock)
                {
            ...
            var promptText = string.Join(" ", text.Select(x => x.Text));
            Task.Run(async () =>
            {
                var aiResult = await this.lmStudioService.GenerateResponse(promptText);
                ...
                var aiExpression = new Expression(aiResult.Trim(), "a");
                lock (this.sayingLock)
                {
                    this.saying.Enqueue(new[] { aiExpression });
                }
            });
        }
        ...
        private async Task SpeakingThread()
        {
            while (this.applicationRunning)
            {
                ...
                else
                {
                    ...
                    // Speak
                    IEnumerable<Expression> text;
                    lock (this.sayingLock)
                    {
                        text = this.saying.Dequeue();
                    }
